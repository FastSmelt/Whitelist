package gg.filerunner.whitelist.profile.constuctor;

import gg.filerunner.whitelist.Whitelist;
import gg.filerunner.whitelist.storage.mongo.MongoRequest;
import gg.filerunner.whitelist.util.atask.TaskUtil;
...
public abstract class PlayerProfile {
    ...
    public final void load() {
        Whitelist.getInstance().getMongoStorage().getOrCreateDocument(collectionName, id, (document, exists) -> {
            if (exists) {
    ...
    public final void save(boolean async) {
        ...
        if (async) {
            TaskUtil.runAsync(Whitelist.getInstance(), request::run);
        } else {
            request.run();
        }
    }
